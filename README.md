...
The application features a beautiful dark neon gradient theme with:
- Cyan, pink, and purple neon accents
...
- All API keys should be stored in `.env` file (never commit to git)
- Auth0 tokens are verified on the backend
- Firestore credentials should be kept secure
...
- The application uses mock data when API keys are not configured
- Replace all "Change" placeholders in `.env` with actual credentials
- Firestore integration requires Firebase service account JSON file

## 🏆 Hackathon Features

This project satisfies requirements for:
- **StateFarm**: Deep comparison + summaries
- **NVIDIA**: Multi-step workflow + tool orchestration (Nemotron)
- **Google**: Best use of generative reasoning (Gemini)
- **Vultr**: Cloud-native deployment ready
- **Auth0**: Enterprise security and audit logging
